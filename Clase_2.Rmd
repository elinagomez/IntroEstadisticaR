...
- 4 secciones de R: consola, script, ambiente y otros 

- Diferencia entre comentarios, calculadora, objetos

- Tipo y clase de objetos
  - Tipo: tipo de datos dentro del objeto (numérico, carácter, lógico)
  - Clase: estructura de datos (vector, matriz, lista, dataframes)

- Coercionadores (as.logical, as.character, as.numeric)
...
## Dataframes 
Una segunda manera para referirnos a datos dentro un dataframe es usando el símbolo `$` . Es la manera más utilizada para refernirnos a una columna de un dataframe, y es muy sencillo de utilizar.  
...
- Leer la documentación de las funciones es fundamental, y sobre todo prestar atención a los argumentos por defecto.
...
Muchas veces podemos no entender que es lo que salió mal y copiar y pegar el error en un buscador de internet es un buen primer paso.
...
vector_1[is.na(vector_2)] # Veo qué valores del vector_1 quedaron como NA en vector_2
vector_1 <- gsub("%", "", vector_1) # Quito los % del vector original 
vector_1 # Sin porcentaje
vector_2 <- as.numeric(vector_1) # Transformo a vector númerico
vector_2 # Sin valores perdidos
```
]
...
- Los paquetes normalmente están relacionados a alguna temática, por ejemplo:
  ...
  - importar y exportar datos [readr](https://cran.r-project.org/web/packages/readr/readr.pdf). 
- Paquetes específicos: 
  -   conectarse con la API de Twitter [rtweet](https://cran.r-project.org/web/packages/rtweet/rtweet.pdf), 
  - utilizar los datos de [Gapminder](https://cran.csiro.au/web/packages/gapminder/gapminder.pdf) 
  -  analizar datos electorales y de opinión pública en Uruguay [opuy](https://github.com/Nicolas-Schmidt/opuy/blob/master/man/figures/Manual_opuy.pdf)
...
## Paquetes
A su vez, una vez instalado un paquete, podemos usar una función específica sin cargar todo el paquete de la siguiente manera:
