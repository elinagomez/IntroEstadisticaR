...
<li> Descarga del EVA del curso las carpetas "Data" y las carpetas "scripts" de los días 1 y 2. Pegalas en la carpeta del punto 1 (guarda todos los scripts dentro de la misma carpeta)
...
```{r ej2b, echo = FALSE, results='hide'}
...
aggregate(
  data[data$presidente %in% c("Vázquez", "Sanguinetti"),
       c("gdp_lcu", "inflation")],
  by = list(presidente = data$presidente[data$presidente %in% c("Vázquez", "Sanguinetti")]),
  FUN = mean,
  na.rm = TRUE
)
```
...
- Existen varios paquetes que permiten importar y exportar datos desde distintos formatos. Algunos de los más utilizados son
  - [readr](https://readr.tidyverse.org/) para archivos csv 
  - [haven](https://haven.tidyverse.org/) para SPSS; STATA 
  - [readxl](https://readxl.tidyverse.org/) para excel 
